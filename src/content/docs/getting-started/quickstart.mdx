---
title: Quickstart
description: One quickstart, two paths — govern a first authority-bound agent with the palonexus Python SDK (10 minutes, fully offline), or bring the PaloNexus platform up on a local kind cluster.
sidebar:
  order: 1.5
---
...
- **Govern an agent (Python SDK)** — register an agent, see a governed call denied by
  default, approve it as a human, and verify the audit chain. About ten minutes, fully
  offline: no cluster, no network, no API key.
- **Run the platform locally** — bring the control layer up on a local
...
In that scenario (from the [seed data](/docs/develop/enterprise-iam/)), a site-reliability
...
<Aside type="note" title="Seeded identities">
The offline control plane seeds a sample identity for each governance role — owner,
sponsor/approver, operator, auditor, and a negative persona that must be denied (used in
step 5). The code below uses these seeded values; a live deployment uses real identities
from the connected Logto organization.
</Aside>

<Aside type="note" title="Supported stack">
PaloNexus currently targets organizations that use [Logto](https://logto.io) as their IAM
solution, run Kubernetes in the cloud, and want to govern Python-based AI agents. The SDK
core models **agents, owners, delegations, tasks, resources, and decisions**, so the
`runbooks` sample resource (`runbooks-api:/runbooks/…`, action `runbooks:read`) is not
special: any resource and scope follow the same calls — swap in `payments-api` /
`payments:write` and the code below is unchanged. The sample service→scope mapping lives in
`palonexus.reference`; the core ships without it.
</Aside>
...
*The quickstart path: install, spin up the offline control plane, register an
authority-bound agent, see the regulated call denied by default, have the approver grant a
...
canonical `PaloNexus.offline()` end-to-end governed flow shown below — pick a persona, edit the governed
call, and run register → deny → delegate → approve → succeed with no `pip install`, no cluster,
and no API key.
</Aside>

The same flow is available in the portal. On a live cluster, the Day-0 onboarding wizard walks an
operator through the same sequence — connect the Logto IdP, seed sample data, register the
first agent, then run the end-to-end governed flow (register → deny → approve → succeed):
...
*The Day-0 wizard's 'Connect Logto' step connects the organization's Logto tenant via
OpenID Connect (OIDC)/SCIM — see
...
The LangChain and LangGraph extras aren't needed for this quickstart — the base package
ships the offline control plane and the end-to-end governed flow.
...
`PaloNexus.offline()` provides an in-memory control plane that reproduces the real
...
This is the core contract: a regulated action can't happen until a human with the
...
person. In a live deployment the approval comes from the approver in the
...
## 5. Test the deny path

Swap the subject for the seeded *negative* persona and the same call is a **hard deny**,
not a needs-approval. No delegation can be requested:
...
The two outcomes are distinct: `needs_approval` means *ask a human*; a flat
...
Open the portal's **Tenant settings** to see the organization defaults applied to every
newly registered agent — the org id, the environment, and the default data-class and risk-tier:
...
- [Temporary elevation walkthrough](/docs/develop/guides/temporary-elevation-walkthrough/) —
  the end-to-end governed flow, narrated.
...
- [Self-host the platform](/docs/operations/self-hosting/)
