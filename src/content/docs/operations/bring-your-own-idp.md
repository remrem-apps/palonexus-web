...
description: Wire the enterprise IdP as the human sign-in issuer for a self-hosted PaloNexus — Logto is the supported IdP; any standard OIDC/SCIM provider (Okta, Entra ID, Auth0, Keycloak) plugs in through the same seam. Agent egress identity (DID/VC) is independent of this.
sidebar:
  order: 4
---
...
**not** depend on the workforce identity provider (IdP) — egress works out of the box. This page is for the *other*
...
this runbook wires the organization's IdP so the control plane verifies inbound human identity against
that issuer.
...
- **Human ingress identity** = OIDC against the workforce IdP. This page. Optional but recommended
...
1. **Create an API resource** named e.g. `PaloNexus` with **API identifier** `palonexus`.
   This identifier is the `OIDC_AUDIENCE` — access tokens must carry it as `aud`.
2. **Create an application** for the human sign-in surface (the portal / the SSO front
...
The control plane now verifies inbound bearer JSON Web Tokens (JWTs) against the IdP's JWKS, requiring
...
subject** (not email). Feed the workforce directory into agent-idp's directory seam so those subjects
resolve:
...
Acceptance: a real login token from the configured IdP is verified at `/authz` (right `iss`/`aud`),
...
- **Logto is the supported IdP** (worked example + seeder). Other OIDC IdPs use the
