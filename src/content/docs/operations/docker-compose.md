...
> `overlays/dev` posture — OpenID Connect (OIDC) sign-in is **off** (anonymous passthrough) but the
...
| `OPA_URL` | _(unset)_ | Inline policy only in compose — no Open Policy Agent (OPA) container. The org-wide Rego veto is a k8s add-on. |
